...
export function LeaderboardTable({ competitionId, competitionName }: LeaderboardTableProps) {
  const { user } = useUser()
  ...
  useEffect(() => {
    ...
    const fetchLeaderboardData = async () => {
      ...
      if (isAdmin && leaderboardData && leaderboardData.length > 0) {
        ...
        if (profilesData) {
          profilesData.forEach((p: any) => { usernameMap[p.profile_id] = p.username })
        }
      }
  ...
  }, [user, role, roleLoading, competitionId])
      ...
      >
        <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {(['all', ...availableCategories] as string[]).map((cat) => (
                  <button
                    ...
                    style={{
                      backgroundColor: categoryFilter === cat ? 'var(--accent)' : 'var(--button-secondary-bg)',
                      color: categoryFilter === cat ? 'var(--accent-text)' : 'var(--button-secondary-text)',
                      borderWidth: '1px',
                      borderStyle: 'solid',
                      borderColor: categoryFilter === cat ? 'var(--accent)' : 'var(--border)',
                    }}
                    onMouseEnter={(e) => { if (categoryFilter !== cat) e.currentTarget.style.backgroundColor = 'var(--button-secondary-hover)' }}
                    onMouseLeave={(e) => { if (categoryFilter !== cat) e.currentTarget.style.backgroundColor = 'var(--button-secondary-bg)' }}
                  >
                    {cat === 'all' ? 'All' : cat}
                  </button>
                ))}
          ...
          <div className="text-sm pt-2 border-t" style={{ borderColor: 'var(--border)', color: 'var(--foreground-secondary)' }}>
            {competitionName} — {filteredCompetitors.length} competitor{filteredCompetitors.length !== 1 ? 's' : ''}
          </div>
        </div>
      </div>
