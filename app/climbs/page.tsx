...
export default function ClimbsPage() {
  ...
  useEffect(() => {
    ...
    // Boulder grade filter — rope climbs don't have a tag colour so they're excluded when a grade is selected
    if (selectedTagColour !== 'all') {
      filtered = filtered.filter(climb =>
        climb.climb_type === 'boulder' && String(climb.tag_colour?.id) === selectedTagColour
      )
    }
    ...
    setClimbs(prevClimbs => {
      return prevClimbs.map(climb => {
        if (climb.id === climbId) {
          ...
          if (sent) {
            if (existingAscent) {
              return { ...climb, ascents: [{ ...existingAscent, sent: true }] }
            } else {
              return {
                ...climb,
                ascents: [{
  ...
  const buildWallGroups = (climbList: any[]) => {
    const grouped = climbList.reduce((acc, climb) => {
      ...
      if (!acc[wallId]) {
        acc[wallId] = { wall: climb.wall, climbs: [] }
      }
    ...
    return (Object.values(grouped) as Array<{ wall: any, climbs: any[] }>).map((group) => {
      const sortedClimbs = [...group.climbs].sort((a, b) => {
        ...
        if (typeof aTag === 'number' && typeof bTag === 'number') return aTag - bTag
        return String(aTag).localeCompare(String(bTag), undefined, { numeric: true, sensitivity: 'base' })
      })
      return { wall: group.wall, climbs: sortedClimbs }
    })
  }

  const wallGroups = buildWallGroups(filteredClimbs)
  ...
  return (
    <main className="px-0 py-4 sm:py-8">
        ...
        {showFilters && (
          <div className="p-4">
            ...
            <div className="mt-3 text-sm" style={{ color: 'var(--foreground-secondary)' }}>
              Showing {filteredClimbs.length} climbs
            </div>
          </div>
        )}
      </div>
      ...
      <ConfirmationModal
        ...
        onConfirm={() => {
          if (unsendClimbId) markClimbAsUnsent(unsendClimbId)
        }}
        title="Unsend Climb?"
...
function ClimbRow({ climb, user, userRole, showPhoto, onImageClick, selectedProfile, onMarkAsSent, onUnsendClick }: any) {
  ...
  const handleSend = async () => {
    if (onMarkAsSent) await onMarkAsSent(climb.id)
  }
